...
**Run this before starting.** On Pi, use the MCP proxy: `mcp({ search: "performance_start_trace" })` or `mcp({ search: "navigate_page" })`. If search returns nothing, STOP—the chrome-devtools server is not configured or Pi needs a restart after installing `pi-mcp-adapter`.

Pi config: `~/.pi/agent/mcp.json` (chrome-devtools with `--autoConnect`). Slash command: `/mcp`.
